...
t = all_t[chosen]
X = all_X[chosen]
error_traj = X .- permutedims(CircadianFVS.ATTRACTOR)
...
# Plot 2: error trajectories x(t) - x*
p_error = plot(
    layout = (7, 3),
    size = (1200, 1500),
    legend = false,
    plot_title = "State errors under FVS $chosen control (x(t) - x*)"
)

for j in 1:21
    plot!(p_error,
          t[1:stride:end],
          error_traj[1:stride:end, j],
          subplot = j,
          linewidth = 1.5,
          title = CircadianFVS.STATE_NAMES[j],
          xlabel = "t",
          ylabel = "error")
    hline!(p_error, [0.0], subplot = j, linestyle = :dash, linewidth = 1.2)
end

display(p_error)
savefig(p_error, "circadian_fvs2_state_errors.png")

println("\nGenerated files:")
println("  $summary_csv")
println("  circadian_fvs2_all_state_trajectories.png")
println("  circadian_fvs2_state_errors.png")
println("\nFinished.")
